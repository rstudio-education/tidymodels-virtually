...
penguins <- 
  readr::read_csv(
    here::here("slides/data/penguins.csv")) %>% 
...
ggplot(small_train, aes(flipper_length_mm, body_mass_g)) +
...
ggplot(small_train, aes(flipper_length_mm, body_mass_g)) +
  geom_segment(aes(x = flipper_length_mm, 
                   ...
                   y = body_mass_g, 
                   yend = predict(lm(body_mass_g ~ flipper_length_mm))), 
               colour = "#E7553C") +
  geom_smooth(method = "lm", se = FALSE, colour = "#4D8DC9") +
  geom_point(size = 3) +  
  geom_point(aes(flipper_length_mm, fitted(lm(body_mass_g ~ flipper_length_mm))),
...
```{r include = FALSE}
rmse_full <- rmse(peng_pred, truth = truth, estimate = .pred) %>% pull(.estimate)
```
...
```{r include = FALSE}
set.seed(100)
small_split_cv <- mc_cv(penguins, times = 1)
rt_spec <- 
  decision_tree() %>%          
...
rt_train <- function(rset) {
  rpart::rpart(body_mass_g ~ flipper_length_mm, 
               data = analysis(rset))
}
...
```{r echo = FALSE}
base_rt_plot <-
  ggplot(rt_preds, aes(x = flipper_length_mm, y = body_mass_g)) +
...
base_rt_plot +
  ...
  geom_line(aes(x=flipper_length_mm, y = .fitted_tree), colour="#CA225E", size=2) 
```
...
```{r include = FALSE}
single_pt <- filter(small_train, body_mass_g == max(body_mass_g))
```
...
```{r echo = FALSE, fig.align='center'}
over2 <- overfit +
  stat_smooth(method = "lm", se = FALSE, formula = y ~ poly(x, 13), colour = "#E7553C", lwd = 1) +
  geom_point(data = single_pt, 
             fill = "yellow", 
             size = 5, 
             shape = 21)
...
```{r echo=FALSE, fig.align='center'}
set.seed(100)
mc_split <- mc_cv(small_peng, times = 1)
poly_rec <- recipe(body_mass_g ~ flipper_length_mm, data = small_train) %>% 
  step_poly(flipper_length_mm, degree = tune())
...
poly_grid <- expand.grid(degree = 1:6)
...
rmse <- list()
for (i in 1:6) {
    mod[[i]] <- lm(body_mass_g ~ poly(flipper_length_mm, i), small_train)
    pred[[i]] <- predict(mod[[i]])
    rmse[[i]] <- rmse_vec(truth = small_train %>% pull(body_mass_g),
                          estimate = pred[[i]])
}
...
set.seed(16)
one_split <- slice(small_peng, 1:30) %>% 
  initial_split() %>% 
  tidy() %>% 
...
```{r include=FALSE}
peng_resid  <- predict(lm_fit, new_data = peng_train) %>% 
  mutate(true_mass = peng_train$body_mass_g)
rmse_train <- rmse(peng_resid, truth = true_mass, estimate = .pred) %>% pull(.estimate)
rmse_test  <- rmse(peng_pred, truth = true_mass, estimate = .pred) %>% pull(.estimate)
```
...
train_lm <- lm(body_mass_g ~ flipper_length_mm, data = small_train)

lm_test_pred <- train_lm %>% 
  broom::augment(newdata = small_test)
  #dplyr::select(body_mass_g, flipper_length_mm, .fitted, .row)
...
top_area <- peng_plots %>% 
  top_n(20, flipper_length_mm) %>% 
  pull(.row)

top_sale <- peng_plots %>% 
  top_n(20, body_mass_g) %>% 
  pull(.row)
...
tidy_ssplit <- small_split %>% 
  ...
  left_join(small_peng, by = c("Row" = ".row")) %>% 
  select(flipper_length_mm, body_mass_g, Data, Row) %>% 
...
split_plots <- 
  ggplot(tidy_ssplit, aes(x = flipper_length_mm, y = body_mass_g)) +
...
```{r echo = FALSE, fig.align='center'}
set.seed(100)
small_strata <- initial_split(small_peng, 
                              strata = body_mass_g, 
                              breaks = 4)
...
strata_split <- small_strata %>% 
  ...
  left_join(small_peng, by = c("Row" = ".row")) %>% 
  select(flipper_length_mm, body_mass_g, Data, Row) %>% 
  mutate(bucket = ntile(body_mass_g, n = 4))

strata_plot <- ggplot(strata_split, aes(x = flipper_length_mm, y = body_mass_g)) +
