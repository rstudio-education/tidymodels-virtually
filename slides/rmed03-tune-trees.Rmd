...
alz_train %>%
  ggplot(aes(x=tau, y=VEGF)) +
  geom_parttree(data = alz_tree_05, aes(fill=Class), alpha = 0.5) +
  geom_jitter(aes(col=Class), alpha=0.7) +
  theme_minimal() +
  scale_color_manual("true", values = c("#1a162d", "#CA225E")) +
  scale_fill_manual("predicted", values = c("#fdf7f9", "#84cae1")) +
...
alz_tree_30 <-
  tree_mod %>% 
  set_args(min_n = 1, cost_complexity = 0, tree_depth = 30) %>%
  fit(Class ~ tau + VEGF, data = alz_train)

## Plot the data and model partitions
alz_train %>%
  ggplot(aes(x=tau, y=VEGF)) +
  geom_parttree(data = alz_tree_30, aes(fill=Class), alpha = 0.5) +
  geom_jitter(aes(col=Class), alpha=0.7) +
  theme_minimal() +
  scale_color_manual("true", values = c("#1a162d", "#CA225E")) +
  scale_fill_manual("predicted", values = c("#fdf7f9", "#84cae1")) +
  guides(fill = guide_legend(reverse = TRUE)) +
  ggtitle("tree_depth = 30")
```
...
Here's a new model plus workflow to work with.
